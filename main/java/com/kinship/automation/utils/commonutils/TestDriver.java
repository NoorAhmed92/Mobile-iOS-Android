...
//        wait.until(elementFoundAndClicked(by));
    ...
    public WebElement waitForElementToBeClickable(final By by, int timeOutInSeconds) {
        WebElement element=new WebDriverWait(DriverManager.getDriver(), timeOutInSeconds)
    ...
    public void waitforElementStaleness(final WebElement element) {
        new WebDriverWait(DriverManager.getDriver(), explicitWait)
                .until(ExpectedConditions.stalenessOf(element));
    }
    ...
    public WebElement waitForElementToBePresent(final By by) {
        WebElement element=  new WebDriverWait(DriverManager.getDriver(), explicitWait)
                .until(ExpectedConditions.presenceOfElementLocated(by));
        return element;
    }
    ...
    public void clickByCoordinates(final By by) {
        ...
        TouchAction touchAction = new TouchAction((AppiumDriver<MobileElement>)DriverManager.getDriver());
...
//    }
    ...
    private Dimension getWindowSize() {
        if (windowSize == null) {
            windowSize = DriverManager.getDriver().manage().window().getSize();
        }
        return windowSize;
    }

    protected void swipe(Point start, Point end, Duration duration) {
        AppiumDriver<MobileElement> d =(AppiumDriver<MobileElement>) DriverManager.getDriver();
...
//            int scrollEnd = screenHeightEnd.intValue();
            ...
            TouchAction swipe = new TouchAction<>((AppiumDriver<MobileElement>)DriverManager.getDriver());
            for (int i = 0; i < num; i++) {
    ...
    public boolean ScrollToElement(By by) {
        ...
        for (int i = 0; i < numberOfTimes; i++) {
            try {
                ...
                new TouchAction((AppiumDriver<MobileElement>)DriverManager.getDriver())
...
//    }
    ...
    public WebElement findElementsById(final By by,int index) {
        WebElement element= DriverManager.getDriver().findElements(by).get(index);
        return element;
    }

    public void switchContext() {
        //"NATIVE_APP" , WEBVIEW_chrome, WEBVIEW_com.gspann.torrid
        Set<String> contextNames = ((AppiumDriver<MobileElement>)DriverManager.getDriver()).getContextHandles();
        String[] arr = new String[contextNames.size()];
        contextNames.toArray(arr);
        for (String c_arr : arr) {
            if(c_arr.trim().equalsIgnoreCase(""))
                ((AppiumDriver<MobileElement>)DriverManager.getDriver()).context("WEBVIEW_Terrace");
        }
    }
...
//    }
    ...
    public boolean isElemneVisibleOnScreen(By locator) {
        ...
        String nature= FrameworkConfigFactory.getConfig().platform().toString();
        if(!isElementPresent(locator)) {
    ...
    public void selectAndClickiOS(String visibleText) {
        ...
        MobileElement pickerWheel =
                (MobileElement) DriverManager.getDriver().findElement(MobileBy.className("XCUIElementTypePickerWheel"));
    ...
    public boolean scrollToElement(By scrollTo, By scrollTill, ScrollDirection direction, double distance, int trail) {
        ...
        for(int i = 1; i <= trail;	i++) {
            if(DriverManager.getDriver().findElements(scrollTo).size() > 0) {
                if(DriverManager.getDriver().findElement(scrollTo).isDisplayed()) {
                    isFound = true;
                    break;
                }
            }
            if(!tillLast) {
                ...
                if(DriverManager.getDriver().findElements(scrollTill).size() > 0) {
                    if(DriverManager.getDriver().findElement(scrollTill).isDisplayed()) {
                        i =  trail - 1;
                        tillLast = true;
                    }
                }
...
//        	Assert.fail("Failed due to : "+desc); //if click action fails no use proceeding further with the test case
    ...
    public void tapAtPoint(Point point) {
        ...
        ((AppiumDriver<MobileElement>)DriverManager.getDriver()).perform(ImmutableList.of(tap));
    }
    ...
    public boolean isDisplayedWait(By by, int iTimeOut) {
        ...
        try {
            Wait<WebDriver> wait = new WebDriverWait(DriverManager.getDriver(), iTimeOut);
