...
public class HumanStuffPage extends WhistleBasePage {
	...
	private By humanIcon = ByT.locator(By.id("Human"),By.xpath("//*[@text='Human']"));
	...
	public boolean isMadeWithForPresent() { return driver.isElementPresent(MadeWithFor);
	}

	@Step("Click on Human Tab")
	public AccountInformationPage clickHumanTab() {
		driver.waitForElementToBeVisible(humanTitle);
		driver.clickLocator(humanTitle);
		return new AccountInformationPage(driver);
	}

	@Step("Click on Human Icon")
	public AccountInformationPage clickHumanIcon() {
		driver.pauseExecutionFor(2);
		driver.clickLocator(humanIcon);
		return new AccountInformationPage(driver);
	}
	public boolean clickOnSafePlaces(){
		driver.clickLocator(safePlaces);
		return true;
	}
}
