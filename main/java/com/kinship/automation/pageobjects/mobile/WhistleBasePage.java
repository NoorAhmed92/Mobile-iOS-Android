...
public class WhistleBasePage {
	...
	public static String token, userId, realtimeUserChannel, deviceSerialNumber, emailAddress;
	...
	private By helpUrl = locator(By.id("TabBarItemTitle"), By.id("com.sec.android.app.sbrowser:id/url_bar_text"));
	...
	{		
		if(req==null)
		{
			...
			req = new RequestSpecBuilder().setBaseUri(FrameworkConfigFactory.getConfig().baseUrlAPI()).setAccept(WhistleConstants.ACCEPT_HEADER)
	...
	public boolean isAndroidBS() {
		return ( FrameworkConfigFactory.getConfig().platform().toString().equalsIgnoreCase("android"));
	}
	...
	public String getHelpUrl() {
		...
		if (FrameworkConfigFactory.getConfig().platform().toString().equalsIgnoreCase("ios")) {
			...
			WebElement urlTextField = ((AppiumDriver) DriverManager.getDriver()).findElement(helpUrl);
			url = urlTextField.getText();
			log.info("URL " + url);
			((AppiumDriver) DriverManager.getDriver()).activateApp("com.whistle.winston-ent");
		} else {
			driver.waitForElementToBeVisible(helpUrl);
